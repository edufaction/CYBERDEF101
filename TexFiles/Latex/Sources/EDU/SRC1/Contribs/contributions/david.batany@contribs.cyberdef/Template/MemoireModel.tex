\usepackage{beamerarticle}
\usepackage{ae}
\usepackage[french]{babel}
\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
\usepackage{lmodern}
\usepackage{tikz} 
\usetikzlibrary{mindmap,trees, backgrounds}
\usepackage{verbatim}
\usepackage{wrapfig}
\usepackage{xpatch}
\usepackage{hologo}
\usepackage{fontawesome}
\usepackage{graphicx} 
\graphicspath{{Pictures/}} 
\usepackage{booktabs} 
\usepackage{setspace}
\setstretch{1,1}
\usepackage{csquotes}
\usepackage{fancyhdr}
\usepackage{makeidx} 
\usepackage{xspace} 
\usepackage{caption}
\usepackage[a4paper, left=2cm, right=2cm, top=2cm, bottom=2cm]{geometry}
\usepackage{hyperref}
%importation de csvsimple transformant un csv en tableau
\usepackage{csvsimple}
% Police par defaut
%\renewcommand{\familydefault}{pag}
%\definecolor{uMainColor}{RGB}{160,0,0} % Color of the article title and section
\definecolor{color2}{RGB}{220,220,220} % Color of the boxes behind the abstract
\usepackage[explicit]{titlesec}

\titleformat{\section}
  {\color{uMainColor}\Large\sffamily\bfseries}
  {}
  {0em}
  {\colorbox{color2!10}{\parbox{\dimexpr\linewidth-2\fboxsep\relax}{\centering\arabic{section}. #1}}}
  []
  
\titleformat{name=\section,numberless}
  {\color{uMainColor}\Large\sffamily\bfseries}
  {}
  {0em}
  {\colorbox{color2!10}{\parbox{\dimexpr\linewidth-2\fboxsep\relax}{\centering#1}}}
  []  
  
\titleformat{\subsection}
  {\color{uMainColor}\sffamily\large\bfseries}
  {\thesubsection}
  {0.5em}
  {#1}
  []
	
\titleformat{\subsubsection}
  {\sffamily\normalsize\bfseries}
  {\thesubsubsection}
  {0.5em}
  {#1}
  []    
	
\titleformat{\paragraph}[runin]
  {\sffamily\small\bfseries}
  {}
  {0em}
  {#1} 

\titlespacing*{\section}{0pc}{1ex plus4pt minus3pt}{10pt}
\titlespacing*{\subsection}{0pc}{1ex plus3pt minus2pt}{10pt}
\titlespacing*{\subsubsection}{0pc}{1ex plus2.5pt minus1.5pt}{10pt}
\titlespacing*{\paragraph}{0pc}{1ex plus2pt minus1pt}{10pt}

% ----------------------------------------------



%\newcommand{\uelogo}{\begin{tikzpicture}[remember picture,overlay]\node at (current page.north west)[xshift=4cm, yshift=-4cm, text opacity=1.0]{\includegraphics[width=0.1\paperwidth]{template/MemoirePictures/shield-20.pdf}}; 
%\end{tikzpicture} }


%-------------------------------------------------------------------------
%	HEADERS AND FOOTERS
%-------------------------------------------------------------------------

\pagestyle{fancy} % Enable the custom headers and footers
\fancyhead{}
\fancyfoot{}
\renewcommand{\headrulewidth}{1pt}
\fancyhead[L]{\includegraphics{logo-sec101.png}}
\fancyhead[C]{\textbf{SEC101  - Partie 3 : VAR  \& SECOPS – Fiche Technique}}
\fancyfoot[C]{\textbf{page \thepage}}
\setlength{\headheight}{45pt} 


%-------------------------------------------------------------------------
%	COMMANDS
%-------------------------------------------------------------------------

\ifdefined\PRZMODE

\newcommand{\picframe}[4]
{
\begin{frame}
\frametitle<presentation>{#2}
  \begin{center}
	 \includegraphics[height=0.95\textheight, width=0.95\textwidth, keepaspectratio ]{\includepicturesubpath{#1.pdf}}
  \end{center}
\end{frame}

}
\else
\newcommand{\picframe}[4]
{
\begin{figure}[!h]
\centering \includegraphics[width=#3\textwidth]{\includepicturesubpath{#1.pdf}}
\caption{#2 \label{#4}}
\end{figure}
}
\fi


  \newcommand{\link}[2]
  {\href{#1}
  {#2~\raisebox{-0.2ex}{\faExternalLink}\footnote{\url{#1}}}
  }
