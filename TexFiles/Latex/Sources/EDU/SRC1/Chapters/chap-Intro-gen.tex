...
\mode<all>{\picframe{../Latex/Sources/EDU/SRC1/Pictures/img-4model}{Cybersécurité, un domaine hollistique}{0.7}{lbli4model}}

%Le domaine de la sécurité couvre 4 grands axes, Normatif, contractuel, réglementaire et législatif comme par exemple les normes ISO la réglementation RGPD, les règlements intérieurs comme la PSSI (politique de sécurité). Méthodologique comme les analyses de risques. et les technologies avec toutes les solutions pour contrer les attaques ou déployer les différentes protections.
...
La cybersécurité ou la sécurité du numérique\footnote{Historiquement d'autres termes sont utilisés comme  \uac{aSSI} ou  Sécurité Informatique} peut être découverte par de nombreuses voies.
...
\begin{itemize}
  \item Les challenges comme les \UKword{\uac{aCTF}}, ou les \UKword{\uac{aDTF}} qui permettent de mettre le pied dans les techniques et stratégies d'intrusion pour les pentesteurs et auditeurs techniques en herbe;
...
\end{itemize}
\audionote{Test}{Ceci est une note}
\end{frame}
% end PRZ====================
...
\mode<all>{\picframe{../Latex/Sources/EDU/SRC1/Pictures/img-cycle}{Processus Cyber d'entreprise}{0.7}{lblcycle}}
...
\mode<all>{\picframe{../Latex/Sources/EDU/SRC1/Pictures/img-risk-assess}{le cyber-risque}{0.8}{lblriskassess}}
...
\mode<all>{\picframe{../Latex/Sources/EDU/SRC1/Pictures/img-risk-prison}{La menace, une vision de l'attaquant}{0.8}{lblriskjail}}
...
A partir de mai 2018, une responsabilité plus juridique liée à la protection des données a été rendue plus visible avec la nécessité de disposer d'un \uac{aDPO} en entreprise (Data Protection Officer) héritier en France  \textit{\uac{aCIL}}.
Nous n'aborderons pas la fonction, les missions et la dynamique de responsabilité du \uac{aDPO} ici. Il faut toutefois que cette fonction possède de nombreux recouvrements dans la chaine de gouvernance du risque \g{informatique} auprès des directions d'entreprise. Orienté vers la protection des données à caractère personnel, le tropisme de la fonction \uac{aDPO} peut conduire certaines structures à oublier des pans importants des risques numériques comme :
...
Pour les entreprises traitant des informations classifiées de défense ou liées aux contraintes de la classification de l'état, il est indispensable de se doter d'une fonction \uac{aOS} de défense. Son rôle est de s'assurer de la conformité à l'Instruction Générale Interministérielle IGI 1300 pour le \g{Confidentiel Défense} et l'instruction Interministérielle II901 pour le \g{Diffusion Restreinte}.
...
Quand nous aborderons des sujets en forte adhérence avec les dynamiques de la \uac{aGDPR} nous donnerons les liens et les indications adaptés pour les \uac{aDPO}.
Par exemple, nous aborderons l'usage des données nominatives collectées et traitées dans les \uac{aSIEM}, ou celles recueillies sur le DarkWeb etc ...
...
Le maintien des conditions de sécurité du système d'information des grandes entreprises nécessite un \uac{aRSSI} Central ou une fonction semblable rattachée à un niveau plus global de l'entreprise.
On découvre ainsi des  \uac{aRSSI} rattachés à la direction des risques, la direction générale, au contrôle interne …\\
...
Le fin mot de l'histoire est le \g{R} de  \uac{aRSSI}. Son domaine de responsabilité dépendra de son mandat pour assumer ce rôle de garant d'un environnement \g{possédant } des bonnes conditions de sécurité.
La gouvernance de la sécurité, est au coeur du métier du  \uac{aRSSI}. Cette discipline que ce dernier pilote dans l’entreprise se nomme \uac{aGRC}.
...
Bien entendu en fonction de la taille de l'entreprise et de ses enjeux, on peut disposer au sein de l'entreprise de nombreuses personnes ayant une fonction de \uac{aRSSI}. 
...
Les conditions de sécurité représentent les propriétés fondamentales du SI, appelées : \uac{aDICT} , qui favoriseront le fonctionnement optimisé du SI et éviteront l'avènement d'incidents de sécurité irréversibles ou même gênants pour son fonctionnement. D'un certain point de vue, les conditions de sécurité représentent le paramétrage du SI pour lequel le système fonctionne bien dans des conditions de sécurité \g{connues et approuvées}.

Ces fameux critères \textbf{\uac{aDICT}} ou propriétés de sécurité des systèmes d'information visent les objectifs suivants :
%-----------------------------------------
...
l'identification des utilisateurs est fondamentale pour gérer les accès aux espaces de travail pertinents et maintenir la confiance dans les relations d'échange. On voit aussi dans la littérature la terminologie \g{critères \uac{aACID} (Authentification, Confidentialité, Intégrité, Disponibilité)}.
\end{nota}
%-----------------------------------------
...
\mode<all>{\picframe{../Latex/Sources/EDU/SRC1/Pictures/img-cyclevie}{Cycle de vie sécurité dans les projets}{0.8}{lblcycledevie}}
...
{
\begin{itemize}
\item Identifier ses cyber-risques sur la base de méthodologies que l’on retrouve dans l’environnement ISO/CEI 27001/27005 mais aussi sur la méthodologie \uac{aEBIOS} de l’ANSSI (Méthode \uac{aEBIOS} RM en particulier); 
